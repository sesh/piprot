...
from piprot.piprot import parse_file, parse_req_file
...
class TestRequirementsParser(unittest.TestCase):
    ...
    def test_requirement_exact(self):
        ...
        d = parse_req_file(f)
        self.assertEqual(d[0][0], 'pip')
        self.assertEqual(d[0][1], 'requests')
        self.assertEqual(d[0][2], '1.2.3')
        self.assertEqual(d[0][3], False)
    ...
    def test_requirements_ignore(self):
        ...
        self.assertEqual(d[0][1], 'requests')
        self.assertEqual(d[0][2], '1.2.3')
        self.assertEqual(d[0][3], True)

    def test_requirements_file(self):
        with open(
            os.path.join(os.path.dirname(__file__), 'files/_develop.txt')
        ) as f:
            d = parse_file(f, verbatim=False)
            self.assertEqual(d[0][0], 'pip')
            self.assertEqual(d[0][1], 'ipython')
            self.assertEqual(d[0][2], '1.1.0')
    ...
    def test_requirements_file_verbatim(self):
        with open(
            ...
            d = parse_file(f, verbatim=True)
            comments = [x[2] for x in d if not x[1]]
    ...
    def test_environment_file(self):
        with open(
            os.path.join(os.path.dirname(__file__), 'files/_develop.yml')
        ) as f:
            d = parse_file(f, verbatim=False)
            self.assertEqual(d[0][0], 'conda')
            self.assertEqual(d[0][1], 'ipython')
            self.assertEqual(d[0][2], '1.1.0')

            self.assertEqual(d[1][0], 'pip')
            self.assertEqual(d[1][1], 'requests')
            self.assertEqual(d[1][2], '2.12.4')
