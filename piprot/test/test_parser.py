...
class TestRequirementsParser(unittest.TestCase):
    ...
    def test_requirements_file(self):
        with open(os.path.join(os.path.dirname(__file__), 'files/_develop.txt')) as f:
    ...
    def test_recursive_requirements_file(self):
        with open(os.path.join(os.path.dirname(__file__), 'files/test-requirements.txt')) as f:
            d = parse_req_file(f, verbatim=False)
            reqs = [x[0] for x in d]
